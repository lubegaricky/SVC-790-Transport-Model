...
knitr::knit_hooks$set(output = function(x, options) {
  if (!is.null(n <- options$linewidth)) {
    # Wrap each line individually -- NOT `ifelse(nchar(x) > n,
    # strwrap(x, width = n), x)` on the whole vector at once: strwrap()
    # reflows a multi-line vector into a *different-length* result, so
    # ifelse's recycling silently scrambles the output whenever any line
    # is long (this corrupted raw kable()/LaTeX table output, which has
    # long lines, while ordinary short console text never triggered it).
    lines <- strsplit(x, "\n")[[1]]
    lines <- unlist(lapply(lines, function(l) {
      if (nchar(l) > n) strwrap(l, width = n) else l
    }))
...
# Keep each per-model output block (heading + table/printout) together on
# one page in the PDF: request `n_lines` of vertical space right before the
# block starts (via the `needspace` LaTeX package, loaded in preamble.tex);
# if that much room is not left on the current page, LaTeX starts a new
# page *before* the block instead of splitting it midway. This inserts one
# standalone raw-LaTeX line (never a `\begin{}...\end{}` pair, which pandoc
# would otherwise swallow whole -- including the markdown after it -- as an
# opaque raw block). No-op for HTML output, where page breaks do not apply.
needspace <- function(n_lines) {
  if (knitr::is_latex_output()) {
    cat(sprintf("\\needspace{%d\\baselineskip}\n\n", n_lines))
  }
}

# knitr::kable() with no explicit `format` is re-typeset by *pandoc* (not
# kable) as a multi-page-capable `longtable` when the output is PDF. That
# is fine on its own, but a `longtable`'s page-break bookkeeping needs
# several stable compilation passes -- and a `\needspace`-forced break
# sitting right before/between a run of them (as in the six-model loops
# below) corrupts that bookkeeping, silently dropping whole rows or model
# blocks. Forcing plain `tabular` (format="latex") sidesteps this: it is
# one atomic, unbreakable box, so it is both immune to that corruption and
# a better match for "never split a model's table across a page" anyway.
kable_safe <- function(x, ...) {
  if (knitr::is_latex_output()) {
...
kable_safe(vif_table_baseR_display, row.names = FALSE, escape = FALSE,
...
kable_safe(vif_table_car, row.names = FALSE,
...
for (nm in names(models)) {
  ...
  needspace(length(summary_lines) + 5)
...
for (nm in names(models)) {
  ...
  print(kable_safe(tidy_tbl, row.names = FALSE))
  cat("\n")
  ...
  print(kable_safe(glance_tbl, row.names = FALSE, escape = FALSE))
  cat("\n\n")
}
```
...
kable_safe(comparison_display, row.names = FALSE, escape = FALSE,
...
kable_safe(comparison_broom_display, row.names = FALSE, escape = FALSE,
...
for (nm in names(models)) {
  ...
  cat("**Model", nm, "**\n\n")
  print(kable_safe(flags, row.names = FALSE))
  cat("\n\n")
}
```
...
cat(sprintf(
  paste0("Model A (single predictor Nr_People_H) already explains ",
         "$R^2$ = %.3f ($R^2_{adj}$ = %.3f).\n\n"),
...
cat(sprintf(
  paste0("Among the reduced (non-D) models, Model %s achieves the highest ",
         "$R^2_{adj}$ (%.3f) using only %d predictor(s): %s.\n\n"),
  best_reduced$Model, best_reduced$Adj_R2, n_best_reduced_preds,
  best_reduced$Predictors))

cat(sprintf(
  paste0("Model D (all 8 predictors) reaches $R^2_{adj}$ = %.3f -- a gain of just ",
...
if (length(high_vif_predictors) > 0) {
  cat(paste0(
    ...
    "significant despite the higher $R^2$.\n\n"))
} else {
  cat(paste0(
...
cat(sprintf(
  paste0("Given that Model %s recovers %.1f%% of Model D's $R^2_{adj}$ ",
...
# S0 Scenario
## Trip Generation (Forecast Year 2035)

```{r S0 Trip Data}
zone <- 1:8
hhs25 <- c(5470, 1400, 7340, 2500, 5800, 9900, 550, 7550)
jobs25 <- c(1160, 1800, 6500, 25240, 5990, 2000, 12700, 2500)
g_hh <- c(2.1, 2.5, 3.2, 3.2, 3.2, 2.5, 2.1, 4.2) / 100
g_jobs <- c(1.5, 1.9, 2.0, 2.4, 2.0, 2.0, 3.5, 2.0) / 100
nr_people <- c(1.5, 2.5, 2.2, 3.6, 2.2, 3.2, 1.5, 3.6)
workers_ft <- c(1.1, 0.52, 0.46, 0.95, 0.46, 0.79, 0.31, 0.95)
learners <- c(0.35, 0.1, 0.09, 0.15, 0.09, 0.13, 0.06, 0.15)

rate <- 0.811424 + 0.148495*nr_people + 0.271651*workers_ft + 1.036471*learners
hhs35 <- hhs25 * (1 + g_hh)^10
jobs35 <- jobs25 * (1 + g_jobs)^10

P25 <- rate * hhs25
P35 <- rate * hhs35
scale35 <- sum(P35) / sum(jobs35)
A35 <- jobs35 * scale35

result <- data.frame(zone, hhs35 = round(hhs35,1), jobs35 = round(jobs35,1),
                      P35 = round(P35,1), A35 = round(A35,1),
                      P35_peak = round(P35*0.4,1), A35_peak = round(A35*0.4,1))
print(result)
cat("Base 24h P total:", sum(P25), "\n")
cat("2035 24h P total:", sum(P35), sprintf(" (%.2f%% growth)\n", (sum(P35)/sum(P25)-1)*100))
cat("2035 peak P = A total:", sum(P35*0.4), "\n")
```
